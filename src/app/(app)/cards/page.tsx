...
export default function CardsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [{ data: cardsData }, { data: txnsData }] = await Promise.all([
        supabase.from("credit_cards").select("*").eq("is_active", true).order("created_at", { ascending: false }),
        supabase.from("transactions").select("id, description, amount, card_id").order("date", { ascending: false }).limit(100),
      ])
      setCards(cardsData ?? [])
    ...
    <div className="p-4 lg:p-6 space-y-5 max-w-4xl mx-auto">
      ...
      {loading ? (
        <div className="rounded-3xl border bg-card p-12 text-center">
          <p className="text-muted-foreground text-sm">Carregando cartões...</p>
        </div>
      ) : cards.length === 0 ? (
        <div className="rounded-3xl border bg-card p-12 text-center">
          <p className="text-5xl mb-3">💳</p>
          <p className="font-semibold mb-1">Nenhum cartão cadastrado!</p>
          <p className="text-sm text-muted-foreground mb-4">Adicione seus cartões para acompanhar faturas e limites.</p>
        ...
        <div className="space-y-4">
          ...
          {cards.map((card: any, idx: number) => {
            const usedPercent = card.limit_total > 0 ? Math.min((card.limit_used / card.limit_total) * 100, 100) : 0
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const cardTxns = transactions.filter((t: any) => t.card_id === card.id)
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  style={{ background: `linear-gradient(135deg, ${card.color ?? "#8b5cf6"}ee, ${card.color ?? "#8b5cf6"}99)` }}
                >
                  ...
                  <div className="flex items-end justify-between relative">
                    <div>
                      ...
                      <MoneyText value={card.limit_used ?? 0} size="xl" className="text-white font-black" />
                    </div>
                  ...
                  <div className="flex items-center gap-5">
                    <div className="relative shrink-0">
                      <svg width="72" height="72" viewBox="0 0 72 72">
                        ...
                        <circle
                          ...
                          stroke={usedPercent > 80 ? "#ef4444" : usedPercent > 60 ? "#f59e0b" : (card.color ?? "#8b5cf6")}
                          strokeWidth="6"
                          strokeLinecap="round"
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold"><MoneyText value={card.limit_used ?? 0} size="sm" /></span>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold"><MoneyText value={card.limit_total ?? 0} size="sm" /></span>
                      </div>
                    </div>
                  </div>
